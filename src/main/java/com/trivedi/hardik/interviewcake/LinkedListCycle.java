...
public class LinkedListCycle {
	 ...
	 * 1 node ahead of slowRunner, since their speeds differ by only 1. So we would
	 ...
	 * means they would be at the same node! So fastRunner didn't skip over
	 * slowRunner! (This is a proof by contradiction.)
	 * 
	 * Since fastRunner can't skip over slowRunner, at most slowRunner will run
	 * around the cycle once and fastRunner will run around twice. This gives us a
	 * runtime of O(n)O(n)O(n).
	 * 
	 * For space, we store two variables no matter how long the linked list is,
	 * which gives us a space cost of O(1)O(1)O(1).
	 */
